...
2. `DOCKER_TOKEN`: Allows GitHub Actions to push to the [neurogenomicslab DockerHub](https://hub.docker.com/orgs/neurogenomicslab) account.  

^ Ask a fellow [Neurogenomics Lab](https://www.neurogenomics.co.uk/) member for these two tokens. 

3. `CODECOV_TOKEN`: Allows [code coverage](https://app.codecov.io/) results to be displayed via the `badger::badge_codecov()` at the top of the *README.Rmd*. This token is repository-specific, 

^ You can get the [Codecov](https://app.codecov.io/) token from [here](https://app.codecov.io/gh/`r owner`/`r pkg`). 

### GitHub Pages

The [GitHub Actions](https://docs.github.com/en/actions/learn-github-actions/understanding-github-actions) workflows included in this template will automatically create a documentation website for your package. This requires several initial setup steps (you only need to do these once per repo):  
